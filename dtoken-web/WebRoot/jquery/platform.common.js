...
$(function(){
	...
	$("#modifyBtn").click(function(){
		if (typeof(updateAction) == "function"){
			...
			var selectId=null;
			if($("#list2").length>0){//get grid list selected id
				selectId=$.gridSel("#list2");
			}else if($("#list").length>0){//get grid list selected id
				selectId=$.gridSel("#list");
			}
	...
	$.gridSel=function (listId){
		...
		if(typeof($(listId))=="object"){//get grid list selected ids
				selectIds=$(listId).jqGrid("getGridParam", "selrow");
				gridIds=($(listId).jqGrid("getRowData",selectIds).id)+"";
		...
		if(typeof($(listId))=="object"){//get grid list selected ids
			...
			if($(listId).jqGrid("getGridParam", "multiselect")){
				...
				$.each(selectIds, function(key, val) {
					...
					gridIds +=($(listId).jqGrid("getRowData",val).id)+"";
